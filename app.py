...
def get_ai_target_ratios(vix, q_rsi, s_rsi):
    mode = "Normal"; t_qqqm = 40; t_spym = 30; t_sgov = 25; t_qld = 5
    if vix > 30 or (q_rsi < 30 and vix >= 18) or (s_rsi < 30 and vix >= 18):
        mode = "Fear (Tactical Strike)"; t_qqqm = 35; t_spym = 25; t_sgov = 20; t_qld = 20
    elif q_rsi > 70 or s_rsi > 70:
        mode = "Greed (Profit Take)"; t_qqqm = 32; t_spym = 28; t_sgov = 40; t_qld = 0
...
# ==========================================
@st.dialog("🛡️ Project Aegis 전략 가이드", width="large")
def show_strategy_guide():
    st.markdown("""
## 🛡️ Project Aegis — 운영 매뉴얼

> **핵심 철학 (Core Philosophy)**
> 매월 현금을 적립식(**DCA**)으로 넣되, **가치(value)가 시간(time)보다 우선**한다.
> 비쌀 땐 참고, 쌀 땐 공격적으로 줍는다. 미국 대형주에 **의도적으로 집중(intended concentration)**하되, SGOV로 최소 방어선을 지킨다.
...
| 종목 | 역할 | 성격 |
|---|---|---|
| **QQQM** | 성장 (Growth) | 나스닥100 코어 |
| **SPYM** | 안정 (Stability) | S&P500 코어 |
| **QLD** | 전술 위성 (Satellite) | 2배 레버리지, 폭락장 공격용 |
| **SGOV** | 방어 (Defense) | 초단기 국채, 현금 파킹 |
| **GMMF** | 기타 (Misc) | 보조 |

⚠️ **집중 위험 (Concentration)**: QQQM·SPYM·QLD는 모두 美 빅테크라 사실상 한 방향 베팅이다. 이는 *분산이 아닌 의도된 집중*이며, 그래서 SGOV 방어선을 항상 유지한다.

---

### 🧠 AI 오토파일럿 (3-Mode Autopilot)
시장 상황에 따라 목표 비중이 자동 전환된다.

| 모드 | 발동 조건 | QQQM / SPYM / SGOV / QLD |
|---|---|---|
| **Normal** | 평상시 | **40** / 30 / **25** / **5** |
| **Fear (Tactical Strike)** | VIX>30 또는 (RSI<30 & VIX≥18) | 35 / 25 / **20** / **20** |
| **Greed (Profit Take)** | RSI>70 | 32 / 28 / **40** / 0 |

→ 평시 주식 노출 75%(공격적 배분). 공포장에선 QLD 20%로 타격하되 **총량 상한 30%**를 넘지 않음. 과열장에도 SGOV 40%까지만 후퇴.

⚠️ **QLD는 5% 고정**: 2배 레버리지는 매일 재조정되어 횡보장에서 가만히 있어도 가치가 깎입니다(변동성 손실). 지수가 제자리로 와도 QLD는 회복되지 않으므로, 공격성은 QLD가 아니라 QQQM 비중으로 표현합니다.

---

### 🔥 Aegis Master Score (매수 신호 엔진)
6개 요소를 합산해 **100점 돌파 시 긴급 매수**, 70점 이상은 '기회 근접'.

- **Score A — 시장 기회 (Max +60)**: RSI·VIX·200일선. 단 **VIX≥18 교차검증** 통과해야 RSI 점수 인정 *(떨어지는 칼날 방지)*.
- **Score B — 비중 갭 (Max +30)**: 목표보다 쪼그라든 종목에 가산점.
- **Score C — 시간 압박 (Max +50)**: 매월 5일 기준, 날이 갈수록 상승하는 **DCA 엔진**. (현금 10만~60만원 구간에서 부드럽게 증가)
- **Score D — 환율 페널티 (Max −50)**: 환율이 (내 평단×0.15 + 60일평균×0.85)보다 비싸면 감점. 달러 강세(DXY↑)면 페널티 완화. *평단 가중치를 낮춰 과거 고환율 앵커 효과를 줄임.*
- **Score E — 과열 페널티 (감점)**: RSI>55거나 200일선 +10% 위면 감점. **단 RSI 낮은 폭락장엔 0점** → 공포매수는 살아있음. *(비싼 장 줍줍 방지의 핵심)*
- **Score F — 저환율 보너스 (Max +15)**: 환율이 기준선보다 싸면 가점. 상한을 15점으로 조여, 환율만으로 100점을 넘지 않게 함.

> 💡 **A vs E의 균형**: A가 "싸니까 사라" 점수를 주면, E는 "비싸니까 참아라"로 깎는다. 시간(C)이 가치 판단을 덮어쓰지 못하게 하는 안전장치.

---

### 🤖 텔레그램 봇 알림 (Bot Signals)
- **긴급 매수/환전**: 3종목 중 최고점 1개만 콕 집어 지시.
- **진성 하락장 매수**: VIX 교차검증 통과 시 QQQM/SPYM/QLD 단계별 공격.
- **추세 필터 (Trend Filter)**: QQQM이 200일선 아래(하락추세)면 매수 강도를 **절반**으로 쪼개 분할 진입. **QLD도 동일 적용** (하락추세 25% / 평시 50%) — 2배 레버리지는 물타기로 평단을 낮춰도 변동성 손실 때문에 회복이 어렵기 때문.
- **현금 천장 (Cash Ceiling 35%)**: 달러 현금이 35% 넘으면 노는 돈으로 보고 SGOV 파킹 권장.
- **매수 가능 수량**: 모든 매수/파킹 알림에 **"○주 매수 가능"** 정확한 정수 주식 수 표시.
- **매도 안전장치**: 모든 매도 지시는 **실제 보유 수량을 초과하지 않음**.

---

### 👮 세금 & 절세 (Tax)
- **세금 지킴이**: 올해 실현수익·남은 비과세(250만원)·예상세금(초과분 22%) 추적.
- **손실 수확 (Tax-Loss Harvesting)**: 수익이 250만원 넘으면, 손실 종목을 팔아 상쇄해 아낄 세금을 자동 계산. 연말(11~12월)엔 강조 알림. *(한국 해외주식 손실은 이월 불가)*

---

### 🛟 안정성 장치 (Safety)
- **환율 실패 보호**: 야후 환율 수신 실패 시 1450원을 캐시에 굳히지 않음 → 다음 새로고침에 재시도.
- **봇 작동 중단**: 데이터 수신 실패 시 0으로 계산하지 않고 봇을 멈춤 → **잘못된 매수 차단**.
- **세금 거짓폭탄 방지**: 환율 0짜리 매수 기록은 세금 계산에서 제외.
- **같은 날 정렬**: 매수를 매도보다 먼저 처리해 원가 꼬임 방지.

---

⚠️ **면책 (Disclaimer)**: 본 시스템은 투자 보조 도구이며, 모든 수치(점수 기준·비중·천장 등)는 검증된 최적값이 아닌 합리적 출발점입니다. 최종 투자 판단과 책임은 본인에게 있습니다.
    """)
...
with st.expander("📖 Aegis Master Score 작동 원리 (Introduction)"):
    ...
    **Project Aegis**는 매월 일정한 현금 흐름을 바탕으로 우량 ETF를 모아가는 장기 퀀트 시스템입니다.
    봇은 6가지 핵심 요소를 실시간으로 계산하여 **총점 100점** 돌파 시, 비싼 환율과 수수료를 감수하고 전략적 매수를 강행합니다.
    ...
    * **📉 환율 페널티 (Max -50점) / 저환율 보너스 (Max +15점):** 기준선(평단×0.15 + 60일평균×0.85)보다 비싸면 감점, 싸면 가점.
